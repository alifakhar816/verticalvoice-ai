...
- **`phi_mode` stays `false`.** The flag is hardcoded to `false` in
  `src/config/features.ts` regardless of environment variables, so it cannot
  be flipped on by an env var alone. Enabling it for real requires a code change
...
3. **Do not run `supabase/seed.sql` against production.** Seed data
   ...
   (`demo@verticalvoice.ai`), and it is for local and staging only. To seed
   staging:
   ```bash
...
3. Click the "..." menu → **Promote to Production** (Vercel's instant
   ...
   rebuild required, and is typically live within seconds.
...
since rollback is itself a new, auditable version. The action is logged to
