...
## Scene 2: Show the architecture, not only the demo (1 minute)
...
three Industry Packs. That is what makes the platform scale to a
...
1. **Healthcare (Sunrise Medical Clinic)**: *"Hi, I need to reschedule my
   ...
   Expect intent `reschedule`, no diagnosis language, and insurance/identity
...
2. **Restaurant (Bellas Italian Kitchen)**: *"Can I get a table for 4 tonight
   ...
   Expect intent `reservation` + `allergen_inquiry`, with allergen info
   provided from menu data and **no guarantee of allergen-free preparation**.

3. **Real Estate (Metro Realty Group)**: *"I'd like to see the 3-bedroom
   listing on Birch Road this Saturday."*
   Expect intent `book_showing`, a listing lookup, and a showing scheduled
   without any commentary on neighborhood demographics.

For each, point at the Text Simulator's output panel: Detected Intent,
Confidence, and Suggested Response. This exercises the same intent
...
## Scene 5: Break it on purpose (safety demo, 1 to 2 minutes)
...
For each, point out that this isn't the LLM "being careful." It is a
...
on top of it. A fourth vertical is a config file away, not a rewrite."*
